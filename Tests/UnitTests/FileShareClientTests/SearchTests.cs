...
namespace UKHO.FileShareClientTests
{
    public class SearchTests
    {
        ...
        public async Task TestEmptySearchQuery()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("");
        ...
        public async Task TestSimpleSearchString()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'");
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
        ...
        public async Task TestSimpleSearchWithDifferentPageSize()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", 50);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
        ...
        public async Task TestSimpleSearchStartingOnNextPage()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", null, 20);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
        ...
        public async Task TestSimpleSearchWithPageSizeAndStartingOnNextPage()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", 10, 20);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
        ...
        public void TestSearchWithInvalidPageSizeThrowsArgumentException(int pageSize)
        {
            var exception = Assert.ThrowsAsync<ArgumentException>(async () =>
                await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", pageSize, 20));
        ...
        {
            var exception = Assert.ThrowsAsync<ArgumentException>(async () =>
                await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", -10, 20));
        ...
        public async Task SearchQuerySetsAuthorizationHeader()
        {
            ...
            await fileShareApiClient.SearchAsync("");
        ...
        public async Task TestEmptySearchQueryWithCancellation()
        {
            ...
            nextResponse = expectedResponse;
            var response = await fileShareApiClient.SearchAsync("",null,null,cancellationToken:CancellationToken.None);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
            Assert.AreEqual("", lastRequestUri.Query, "Should be no query query string for an empty search");
        ...
        public async Task TestSimpleSearchStringWithCancellation()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'",null,null, cancellationToken: CancellationToken.None);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
            Assert.AreEqual("?$filter=$batch(key)%20eq%20%27value%27", lastRequestUri.Query);
            Assert.AreEqual((int)nextResponseStatusCode, response.StatusCode);
            Assert.IsTrue(response.IsSuccess);
        ...
        public async Task TestSimpleSearchWithDifferentPageSizeAndCancellation()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", 50, null , cancellationToken: CancellationToken.None);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
            Assert.AreEqual("?$filter=$batch(key)%20eq%20%27value%27&limit=50", lastRequestUri.Query);
            Assert.AreEqual((int)nextResponseStatusCode, response.StatusCode);
            Assert.IsTrue(response.IsSuccess);
        ...
        public async Task TestSimpleSearchStartingOnNextPageWithCancellation()
        {
            ...
            var response = await fileShareApiClient.SearchAsync("$batch(key) eq 'value'",null, 20 , cancellationToken: CancellationToken.None);
            Assert.AreEqual("/basePath/batch", lastRequestUri.AbsolutePath);
            Assert.AreEqual("?$filter=$batch(key)%20eq%20%27value%27&start=20", lastRequestUri.Query);
            Assert.AreEqual((int)nextResponseStatusCode, response.StatusCode);
            Assert.IsTrue(response.IsSuccess);
        ...
        {
            var exception = Assert.ThrowsAsync<ArgumentException>(async () =>
                await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", pageSize, 20 , cancellationToken: CancellationToken.None));
        ...
        {
            var exception = Assert.ThrowsAsync<ArgumentException>(async () =>
                await fileShareApiClient.SearchAsync("$batch(key) eq 'value'", -10, 20, cancellationToken: CancellationToken.None));
        ...
        public async Task SearchQuerySetsAuthorizationHeaderWithCancellation()
        {
            ...
            await fileShareApiClient.SearchAsync("",null,null, cancellationToken: CancellationToken.None);
